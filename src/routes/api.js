...
export function registerApi(app, mrs) {
  ...
  app.post('/api/actors', async (req, res) => {
    ...
    try { sseHub.broadcastJson('{"type":"actor.changed","op":"create"}'); } catch {}
    res.type('application/json').send(body);
  });
  ...
  app.patch('/api/actors/:id', async (req, res) => {
    ...
    try { sseHub.broadcastJson(`{"type":"actor.changed","op":"patch","id":${JSON.stringify(req.params.id)}}`); } catch {}
    res.type('application/json').send(body);
  });
  ...
  app.put('/api/actors/:id', async (req, res) => {
    ...
    try { sseHub.broadcastJson(`{"type":"actor.changed","op":"put","id":${JSON.stringify(req.params.id)}}`); } catch {}
    res.type('application/json').send(body);
  });
  ...
  app.delete('/api/actors/:id', async (req, res) => {
    ...
    try { sseHub.broadcastJson(`{"type":"actor.changed","op":"delete","id":${JSON.stringify(req.params.id)}}`); } catch {}
    res.type('application/json').send(body);
  });
}
